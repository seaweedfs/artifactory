...
results=Path(os.environ.get('T3_RESULT_ROOT',os.environ['SWEEP_ROOT']))/tag
...
client_host=os.environ['T3_CLIENT_HOST']
client_user=os.environ['T3_CLIENT_USER']
client_key=os.environ['T3_CLIENT_KEY']
client=f'{client_user}@{client_host}'
server_ip=os.environ['T3_SERVER_IP']
server_ssh=os.environ['T3_SERVER_SSH']
rdma_device=os.environ['T3_RDMA_DEVICE']
gid_index=os.environ['T3_GID_INDEX']
...
server_env.write_text('export PATH=/usr/local/go/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin\n'
    ...
    +f'TESTOPS_T3_SERVER_IP={server_ip}\nTESTOPS_T3_DEVICE={rdma_device}\nTESTOPS_T3_GID_INDEX={gid_index}\nCI_PORT_BASE=27000\n')
client_env=results/'client.env'
client_env.write_text('export PATH=/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin\n'
    +f'TESTOPS_ACTIVITY_LOG=/mnt/smb/work/share/testops/WHO-IS-RUNNING\nTESTOPS_T3_DEVICE={rdma_device}\nTESTOPS_T3_GID_INDEX={gid_index}\n')
...
cpu={}
for host in (server_ssh, client):
    cpu[host]=subprocess.check_output(['ssh',host,'cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor /sys/devices/system/cpu/intel_pstate/no_turbo'],text=True).splitlines()
runner=str(Path(os.environ['SWEEP_ROOT'])/'bin/sw-test-runner')
weed=str(Path(os.environ['SWEEP_ROOT'])/'bin/weed')
volume=os.environ.get('SWEEP_VOLUME_BIN','/opt/work/bin/weed-volume-'+PIN+'-rdma')
scenario=dict(name='rdma-integration-t3',timeout='8m',topology=dict(nodes=dict(server=dict(is_local=True),
    client=dict(host=client_host,user=client_user,key=client_key))),
